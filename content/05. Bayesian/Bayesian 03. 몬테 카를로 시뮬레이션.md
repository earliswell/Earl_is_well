...
- 결국, 제안 분포로부터 샘플을 draw하고, 균등 분포에서 draw해서, $u_{j} < f(x_{j})/[cg(x_{j})]$를 계산하게 되는데, 여기에서 $f(x_{j})/[cg(x_{j})]$는 수용률이 됨.
	...
	- $x_{j}$ 또한, 제안 분포로부터 샘플링 된 어떠한 랜덤 변수인데, 이를 우항에 대입하면 다음과 같음. $u_{j} < Constant$. 
	- 여기에서 $u_{j}$는 상수보다 작거나 클 것이고, $u_{j}$는 균등 분포를 따르기 때문에 \[0, 1]까지에서 모든 값들이 동일한 확률로 추출됨. 
	- 이는 무엇을 의미할까? 여기서 상수가 0.5라면 $x_{j}$가 수용될 확률은 $u_{j}$가 0.5보다 작을 확률을 의미하며 $Pr(u_{j}< 0.5)$는 결국 0.5이다. 따라서
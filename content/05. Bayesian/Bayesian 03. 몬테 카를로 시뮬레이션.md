...
3. 각각의 $j$에 대해서 만약 $u_{j} < f(x_{j}) / [cg(x_{j})]$이 만족되면, $x_{j}$를 수용한 뒤 저장한다. 만족되지 않으면 $x_{j}$를 기간한다.
- 결국, 제안 분포로부터 샘플을 draw하고, 균등 분포에서 draw해서, $u_{j} < f(x_{j})/[cg(x_{j})]$를 계산하게 되는데, 여기에서 $f(x_{j})/[cg(x_{j})]$는 수용률이 됨.
	- $f(x_{j})/[cg(x_{j})]$에 대해서 조금만 더 깊이 이해를 해보자. 
	- $x_{j}$는 제안 분포로부터 샘플링 된 어떠한 랜덤 변수인데, 어떠한 특정된 값인 $x_{j}$를 $f(x_{j})/[cg(x_{j})]$에 대입하면 어떠한 값이 나오게 됨. $u_{j} < f(x_{j})/[cg(x_{j})]$. 
	- 여기에서 $u_{j}$는 상수보다 작거나 클 것이고, $u_{j}$는 균등 분포를 따르기 때문에 \[0, 1]까지에서 모든 값들이 동일한 확률로 추출됨. 
	- 이는 무엇을 의미할까? 여기서 상수가 0.5라면 $x_{j}$가 수용될 확률은 $u_{j}$가 0.5보다 작을 확률을 의미하며 $Pr(u_{j}< 0.5)$는 결국 0.5이다. 따라서, 이는 $x_{j}$가 해당 위치에서 수용될 확률이 0.5를 의미함.
#### 3.3.2 이론적 배경
- A-R 기법하의 샘플링 과정에서 → 제안 분포로부터 추출된 확률변수 $x$에 대해 만약 균일 분포로부터 샘플링한 $u$가 $\frac{f(x)}{cg(x)}$보다 작으면 그 때의 $x$를 타깃 분포에서 샘플링한 것으로 간주하였다고 한다.
- 이는 $u < \frac{f(x)}{cg(x)}$라는 조건을 만족하는 제안 분포의 밀도가 타깃 분포의 밀도와 동일하다. 따라서 
$$
...
$$
###### Example 3. 베타 분포 샘플링
- A-R 알고리즘을 적용해서 $Beta(3,3)$ 분포에서 500개 관측치 샘플링 → 평균, 분산 계산하여 실제 $Beta(3,3)$ 분포의 평균과 분산을 비교!
$$
f(x) = \frac{\Gamma(6)}{\Gamma(3)\Gamma(3)}x^{(3-1)}(1-x)^{(3-1)}
$$
- 제안 분포 $g(x)$를 서포트가 동일한 균일 분포 $Unif(0,1)$로 설정
- $c=1.8750$로 할당함.
1. 제안 분포 $g(z)$에서 $z_{i}$ 샘플링.
2. $u_{i} \sim Unif(0,1)$에서 $u_{i}$를 샘플링
- 수용률 : $f(z_{i})/1.8750$이며 이 값이 $u_{i}$보다 크면 수용 작으면 기각함. 
- 관측치가 1,000개가 될 때 까지 반복
- 실제 $Beta(3,3)$ 분포의 평균과 분산은 다음과 같음
$$
\begin{align}
 E(X) & = \frac{\alpha}{\alpha+\beta} = 0.5, \\
 Var(X) & =\frac{\alpha \beta}{(\alpha +\beta)^2(\alpha+\beta+1)} = 0.0357
\end{align}
$$
- 실제 코드로 돌려보면 평균과 분산은 각각 0.5003과 0.0355로 계산됨! → 시뮬레이션 크기가 클수록 감소함.
###### 알고리즘 3.6: 베타 분포 샘플링
...
- 우리는 Importance Sampling 기법의 작동원리를 좀 더 깊이 이해하기 위해서 $f(z_{i})/h(z_{i})$의 역할에 대해서 더욱 깊이 생각해볼 필요가 있다!
	- $f(z_{i})/h(z_{i}) > 1$이라면, $f(z_{i})$가 큰 값이거나 $h(z_{i})$가 매우 작은 값임을 의미함. 즉 $z_{i}$라는 확률변수가 $f(\cdot)$에서 발현될 확률이 더 크다는 의미를 지님.
	- 반대로 $f(z_{i})/h(z_{i}) < 1$이라면, $f(z_{i})$가 매우 작은 값이거나 $h(z_{i})$가 큰 값임을 의미함. 이는 확률변수 $z_{i}$가 상대적으로 $h(z_{i})$에서 발현될 확률이 더 큼.
	- 이렇게 확률변수 $z_{i}$에 가중치를 넣어 최종적으로 우리가 구하고 싶은 기댓값을 최대한 근사(Approximation)하는 기법임.
- 이는 결국에는 Importance Sampling Function의 역할이 중요함. → 제대로 선택하지 못하면 정확한 몬테 카를로 추정치를 얻을 수 없음.
---
- A-R 기법과 Importance 샘플링은 다변수 일 때, 특히 타깃 분포의 차원이 클 때는 적응하기 힘든 기법임. 타깃 분포의 차원이 크면, 파라미터의 수가 많아짐.
	- A-R 기법에서는 상수 $c$를 구하기 어려움.
- 실제로 시뮬레이션에 필요한 계량 모형의 사후 분포는 대부분의 경우 다차원이기 때문에 몬테 카를로 시뮬레이션 기법을 적용하는 데 한계가 존재함. 
- 이를 해결하기 위한 방법이 마코프 체인 몬테 카를로(MCMC) 시뮬레이션임. 
- 다양한 MCMC 기법 중에서도 베이지안 통계학에서 가장 널리 사용되는 방법은 Metropolis-Hastings 알고리즘으로, 다음 장에서 만나요~

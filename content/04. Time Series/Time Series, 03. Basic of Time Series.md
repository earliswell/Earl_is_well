...
$$
- 이는 Wald Test를 통해서 위 귀무가설을 테스트할 수 있다. 
	- 만약, 이는 귀무가설을 채택하게 된다면 $Z$는 $Y$값을 예측하는데 어떠한 영향을 미치지 않는 것과 같다!! → $Z_{t}$가 없어도 $Y_{t}$를 설명할 수 있음!!
	- 마치, OLS에서 F-검정과 비슷함.
	- 대립가설을 채택하게 된다면 ? 적어도 하나는 0이 아니다라는 해석이 됨!!
- 이러한 Granger Causality는 Conditional Variance를 설명할 때도 이용할 수 있으며, 즉 $Z_{t}$의 과거 값들이 $Y_{t}$의 분산($Var(Y_{t}|\Gamma^1_{t-1})$) → 변동성에 영향을 미치는 지에 대해서도 이 관계를 파악할 수 있음!

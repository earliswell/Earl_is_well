...
- 위 알고리즘에 대해서 이해하기 위해서 $u \sim Uinf(0, 1)$에서 Draw를 하는 것과 역함수에 $u_{i}$를 대입하는 것에 대해서 이해를 할 필요가 있음.
- 우리는 결국 확률 자체의 값을 Random Draw를 많이 반복해야함. 따라서, $u_{i}$는 $F(X)$의 누적 확률값을 의미함. 따라서 모든 확률의 동일 선상에서 추출될 수 있는 샘플들을 계속 Draw하는 역할을 하게 됨!!
- $u_{i}$를 $F^{-1}(u_{i})$에 대입하면 우리는 그에 해당하는 $x_{i}$를 추출할 수 있음.
- 따라서, 이러한 샘플링 기법은 CDF가 필요함!
...
- 결국, 제안 분포로부터 샘플을 draw하고, 균등 분포에서 draw해서, $u_{j} < f(x_{j})/[cg(x_{j})]$를 계산하게 되는데, 여기에서 $f(x_{j})/[cg(x_{j})]$는 수용률이 됨.
	- $f(x_{j})/[cg(x_{j})]$에 대해서 조금만 더 깊이 이해를 해보면 좋음. 
	- $x_{j}$는 제안분포로 부터 추출된 랜덤 변수이다! 이는 결국 $g(\cdot)$으로부터 생성된 것으로, 만약 분포 $g$가 Uniform을 따른다고 했을 때, 이 범위 안에 있는 $x_{j}$는 모두 동일한 확률로 추출됨. 
	- 이렇게 추출된 $f(x_{j})/[cg(x_{j})]$를 계산할 수 있는데, 이는 결국 $u_{j}$를 수용할 수 있는 비율이 된다. $u_{j}$는 확률 분포의 수직선을 그리게 될텐데 이는 
	- $f(x_{j})/[cg(x_{j})]$의 값이 만약 0.01이라고 했을 때, 이는 결국 $Pr(u_{j} < 0.01) = 0.01$이 된다. $u_{j}$는 균등 분포로부터 추출된 샘플이기 때문에 !
	- 이처럼 $f(x_{j})/[cg(x_{j})]$는 어떠한 랜덤 변수 $u_{j}$의 수용률이라고 볼 수 있지 않을까?
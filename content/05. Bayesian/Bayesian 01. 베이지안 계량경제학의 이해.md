...
- 다음은 베이지안 추정에 있어 사전 분포의 역할에 대해서 보다 쉽게 이해할 수 있도록 여러 가지 사전 분포의 예시를 들어보고자 한다.
	...
	- Case (iii) $a_{0}=b_{0}=100$ 인 경우: 사전 분포의 평균은 0.5이며 사후 분포의 평균 또한 0.5에 가깝다. → 즉 이는 정보의 양이 많아졌다고 생각을 할 수 있음. 빈도 주의의 CLT 처럼 어떤 값을 무한히 반복하다보면 어떠한 점으로 수렴하듯 이라고 생각해보자. 
		- 즉, 사전 정보의 양이 많아 사전적인 믿음이 강하다고 해석할 수 있음 !!
		- 이런 경우에는 자료가 주어지더라도 정보가 충분치 않으면 사후 분포에 별 영향을 주지 못하게 됨.
	- Case (iv) $a_{0}=b_{0}=0$ 그리고 표본 크기가 $10^5$ 인 경우:
